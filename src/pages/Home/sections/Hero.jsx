import portfolioData from "../../../data/portfolioData";

const skills = [
  "WordPress",
  "Site Maintenance",
  "Security Hardening",
  "Quality Assurance",
  "HTML",
  "CSS",
  "JavaScript",
  "React",
];

export function Hero() {
  const { name, bio, email, github, linkedin } = portfolioData;

  return (
    <>
      <div className="bento-grid">
        <div className="bento-card bento-card--hero">
          <div className="hero-card__top">
            <div className="hero-card__photo-wrap">
              <div className="hero-card__photo-placeholder">YN</div>
            </div>
          </div>
          <div className="hero-card__bottom">
            <h1 className="hero-card__name">{name}</h1>
            <p className="hero-card__tagline">
              Crafting reliable web experiences — from WordPress to React.
            </p>
          </div>
        </div>

        {/*CARD 2 — About*/}
        <div className="bento-card bento-card--about">
          <p className="bento-label">About</p>
          <p className="about-card__bio">{bio}</p>
          <a href="/work" className="about-card__cta">
            <span>View my work</span>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
                d="M3 8h10M9 4l4 4-4 4"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </a>
        </div>
        {/* CARD 3 — Stat */}
        <div className="bento-card bento-card--stat bento-card--dark">
          <span className="stat-card__number">150+</span>
          <span className="stat-card__label">
            Production WordPress sites managed
          </span>
        </div>
        {/* CARD 4 — CV */}
        <div className="bento-card bento-card--cv">
          <span className="cv-card__title">CV</span>
          <a href="/cv.pdf" download className="cv-card__btn">
            <span>Download</span>
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
              <path
                d="M8 3v8M4 9l4 4 4-4"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
          </a>
        </div>
        {/* CARD 5 — Work preview */}
        <div className="bento-card bento-card--work">
          <p className="bento-label">Featured work</p>
          <div className="work-card__previews">
            <div className="work-card__preview work-card__preview--1">
              <span>Security hardening</span>
            </div>
            <div className="work-card__preview work-card__preview--2">
              <span>Maintenance workflow</span>
            </div>
            <div className="work-card__preview work-card__preview--3">
              <span>Landing page</span>
            </div>
          </div>
          <a href="/work" className="work-card__link">
            See all work →
          </a>
        </div>
        {/* CARD 6 — Skills */}
        <div className="bento-card bento-card--skills">
          <p className="bento-label">Expertise</p>
          <div className="skills-card__tags">
            {skills.map((skill) => (
              <span key={skill} className="skills-card__tag">
                {skill}
              </span>
            ))}
          </div>
        </div>
        {/* CARD 7 — Social */}
        <div className="bento-card bento-card--social">
          <div className="social-card__links">
            <a
              href={github}
              target="_blank"
              rel="noreferrer"
              className="social-card__link"
            >
              <svg
                width="20"
                height="20"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.741 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
              </svg>
            </a>
            <a
              href={linkedin}
              target="_blank"
              rel="noreferrer"
              className="social-card__link"
            >
              <svg
                width="20"
                height="20"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
              </svg>
            </a>
            <a href={`mailto:${email}`} className="social-card__link">
              <svg
                width="20"
                height="20"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <rect x="2" y="4" width="20" height="16" rx="2" />
                <path d="M2 7l10 7 10-7" />
              </svg>
            </a>
          </div>
          <p className="social-card__note">Let's connect</p>
        </div>
      </div>
    </>
  );
}
